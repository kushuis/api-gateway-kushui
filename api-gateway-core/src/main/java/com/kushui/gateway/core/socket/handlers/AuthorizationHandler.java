package com.kushui.gateway.core.socket.handlers;

import com.kushui.gateway.core.mapping.HttpStatement;
import com.kushui.gateway.core.session.Configuration;
import com.kushui.gateway.core.socket.BaseHandler;
import com.kushui.gateway.core.socket.agreement.AgreementConstants;
import com.kushui.gateway.core.socket.agreement.GatewayResultMessage;
import com.kushui.gateway.core.socket.agreement.ResponseParser;
...
public class AuthorizationHandler extends BaseHandler<FullHttpRequest> {
    ...
    protected void session(ChannelHandlerContext ctx, Channel channel, FullHttpRequest request) {
        ...
        try {
            ...
            // 4. 封装返回结果
            DefaultFullHttpResponse response = new ResponseParser().parse(GatewayResultMessage.buildError(AgreementConstants.ResponseCode._500.getCode(), "AuthorizationHandler中异常网关协议调用失败！" + e.getMessage()));
            channel.writeAndFlush(response);
        }
    }
